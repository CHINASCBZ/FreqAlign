...
#PALETTE: 为每个类指定了一个 RGB 颜色，用于可视化时为每个类别分配颜色。

ORIGIN_IMG_SIZE = (1024, 1024)
INPUT_IMG_SIZE = (1024, 1024)
TEST_IMG_SIZE = (1024, 1024)
#ORIGIN_IMG_SIZE, INPUT_IMG_SIZE, TEST_IMG_SIZE:
# 定义了原始图像、输入图像和测试图像的尺寸，大小为 1024x1024。


# 定义了训练数据增强的函数，使用 Albumentations 库中的变换进行数据增强。
def get_training_transform():
    train_transform = [
        albu.HorizontalFlip(p=0.5),
        albu.VerticalFlip(p=0.5),
        albu.RandomBrightnessContrast(brightness_limit=0.25, contrast_limit=0.25, p=0.25),
        albu.Normalize()    #进行标准化，将图像像素值归一化。
    ]
...
#train_aug(): 该函数将输入的图像和掩膜（mask）转换为 numpy 数组，然后应用定义好的增强操作，并返回增强后的图像和掩膜。
...
#get_val_transform(): 这里定义了验证集的图像增强操作，主要是标准化（Normalize）。
...
def val_aug(img, mask):
    img, mask = np.array(img), np.array(mask)   # 将PIL.Image对象转换为numpy数组
    aug = get_val_transform()(image=img.copy(), mask=mask.copy())
    img, mask = aug['image'], aug['mask']
    return img, mask


#这个类继承自 torch.utils.data.Dataset，是一个用于遥感图像分割任务的数据集类。它负责图像和掩膜的加载、数据增强以及批量处理。
class UAVIDDataset(Dataset):
    def __init__(self, data_root='data/uavid/val', mode='val', img_dir='images', mask_dir='masks',
        ...
        self.img_suffix = img_suffix    # 图片后缀
        self.mask_suffix = mask_suffix
        self.transform = transform  # 数据增强
        self.mode = mode
        self.mosaic_ratio = mosaic_ratio    # mosaic_ratio: 用于控制 mosaic 数据增强的比例。
    ...
    #load_img_and_mask(self, index): 根据给定的索引加载图像和掩膜。图像被打开并转换为 RGB 格式，掩膜则被加载为单通道灰度图（L模式）。
    ...
    #load_mosaic_img_and_mask(self, index): 随机选择四张图像进行拼接，生成一个大的 mosaic 图像，并返回拼接后的图像和掩膜。
...
#UAVIDDatasetV2 类继承自 Dataset，与 UAVIDDataset 类似，但增加了以下功能：
#支持对图像中小物体的裁剪（tiny_object_crop()）。
#通过 class_interest 和 class_ratio 参数指定对某些类别（如车、建筑等）的关注度，增强这些类别的出现概率。
...
#这些函数用于可视化图像、掩膜和分割结果。
#用于显示图像、真实掩膜和预测掩膜（分割结果）的对比。使用 matplotlib 来绘制图像、掩膜和分割结果，并为每个类别显示对应的颜色。
...
#show_seg() 和 show_mask()
# 这两个函数也用于显示分割结果和掩膜，类似于 show_img_mask_seg()，但它们用于不同的显示需求。
